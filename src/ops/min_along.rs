...
impl Tensor {
    pub fn min_along<A: Into<Axis>>(mut self, axis: A) -> Result<Self, Error> {
        ...
        let y = if self.shape[ax] == 1 || self.strides[ax] == 0 {
            ...
            let bytes = match self.bytes.borrow().deref() {
                ...
                BytesPtr::Cuda(x_buf) => {
                    ...
                    let max_value = dtype.max_value().to_string();
                    let min_fn = match dtype {
                        Dtype::Float16 => "__hmin_nan",
                        Dtype::BFloat16 => "__hmin_nan",
                        Dtype::Float32 => "fminf",
                        Dtype::Float64 => "fmin",
                        Dtype::Int8 => "min",
                        Dtype::Int16 => "min",
                    ...
                    let module_name =
                        std::format!("{}min{ax:?}{}", self.build_op_name(), dtype.short_name());
...
extern "C" __global__ void kernel(const size_t *info, const uint8_t *src, uint8_t *dst) {{
    ...
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {{
        __syncthreads();
        if (tid < s) {{
            shr[tid] = {min_fn}(shr[tid], shr[tid + s]);
        }}
    }}
